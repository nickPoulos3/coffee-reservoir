...
public class HoldEmRunner {
	...
	public static void main(String[] args) {
		...
		addPot(5000); p1.bet(500); printPot();
		...
		p5.shoutBestHand();
		decideWinner();
		System.out.println("See cards? Type \"Show\"");
		Scanner s1 = new Scanner(System.in);
		input = s1.nextLine();
		if (input.trim().toLowerCase().equals("show")) {p1.showHand(); p2.showHand(); p3.showHand(); p4.showHand(); p5.showHand();}
		s1.close();
	}

	public static void deal(Deck d) {
		for (Player p : table) {
			for (int j = 0; j < 7; j++) {
	...
	public static void decideWinner() {
		...
		for (Player p : table) {
			...
			bestValue = Math.min(bestValue, Arrays.asList(Hand.winningHands).indexOf(hand));
			if (bestValue != temp) {
				winner = p;
